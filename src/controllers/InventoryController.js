...
module.exports = {
  async list(request, response) {
    ...
    try {
      const char_items = await database.select()//'account_name', 'char_name', 'obj_id', 'object_id', 'item_id', 'count', 'enchant_level'
        .where({ char_name })
        .table('characters')
      ...
      const item_info = await Item.find({ item_id: char_items[0].item_id })
